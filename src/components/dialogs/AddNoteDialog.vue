...
export default {
    ...
    methods: {
        close_dialog() {
            this.toggleNoteDialog = false
        },

        submit(){
            this.$store.dispatch('addNote', this.content)
            this.content = ''
        }
    },
    computed: {
        toggleNoteDialog: {
            get() { return this.$store.getters.note_dialog},
            set(v) { return this.$store.commit('toggleNoteDialog', v)}
        }
    }
}
</script>
